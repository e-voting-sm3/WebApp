...
export default {
  ...
  methods: {
    async fetchData() {
      try {
        const response = await axios.get('http://127.0.0.1:8000/api/auth/candidates', {
          headers:{
        ...
        const meResponse = await axios.post('http://127.0.0.1:8000/api/auth/me', {}, {
          headers:{
            Authorization: 'Bearer ' + localStorage.getItem('token')
          }
        });
    ...
    async vote(candidateId) {
      try {
        const response = await axios.post('http://127.0.0.1:8000/api/auth/votes', {
